...
contract OffchainAggregatorBilling is Owned {
    ...
    mapping(address /* signer OR transmitter address */ => Oracle) internal s_oracles;
    ...
    constructor(
        ...
        setBillingInternal(
            _maximumGasPrice, _reasonableGasPrice, _microLinkPerEth, _linkGweiPerObservation, _linkGweiPerTransmission
        );
        s_linkToken = _link;
    ...
    */
    event LinkTokenSet(LinkTokenInterface indexed _oldLinkToken, LinkTokenInterface indexed _newLinkToken);
    ...
    * to handle misbehaving ERC20 tokens!
    */
    function setLinkToken(LinkTokenInterface _linkToken, address _recipient) external onlyOwner {
    ...
    */
     ...
     * @param linkGweiPerObservation reward to oracle for contributing an observation to a successfully transmitted
     * report, in 1e-9LINK units
     ...
     * @param _linkGweiPerObservation reward to oracle for contributing an observation to a successfully transmitted
     * report, in 1e-9LINK units
    ...
    function setBillingAccessControllerInternal(AccessControllerInterface _billingAccessController) internal {
        AccessControllerInterface oldController = s_billingAccessController;
        if (_billingAccessController != oldController) {
            s_billingAccessController = _billingAccessController;
            emit BillingAccessControllerSet(oldController, _billingAccessController);
        }
    }
    ...
    function owedPayment(address _transmitter) public view returns (uint256) {
        ...
        uint256 linkWeiAmount =
            uint256(s_oracleObservationsCounts[oracle.index] - 1) * uint256(billing.linkGweiPerObservation) * (1 gwei);
    ...
    function payOracles() internal {
        ...
        uint256[maxNumOracles] memory gasReimbursementsLinkWei = s_gasReimbursementsLinkWei;
    ...
    /**
     * @notice withdraw any available funds left in the contract, up to _amount, after accounting for the funds due to
     * participants in past reports
     ...
     * @return availableBalance LINK available on this contract, after accounting for outstanding obligations. can
     * become negative
     */
    ...
    function reimburseAndRewardOracles(uint32 initialGas, bytes memory observers) internal {
        ...
        s_oracleObservationsCounts = oracleRewards(observers, s_oracleObservationsCounts);
        // Reimburse transmitter of the report for gas usage
        require(txOracle.role == Role.Transmitter, "sent by undesignated transmitter");
        ...
        uint256 gasCostEthWei = transmitterGasCostEthWei(uint256(initialGas), gasPrice, callDataGasCost, gasLeft);
        ...
        s_gasReimbursementsLinkWei[txOracle.index] = s_gasReimbursementsLinkWei[txOracle.index] + gasCostLinkWei
    ...
    */
     ...
     */
    function setPayees(address[] calldata _transmitters, address[] calldata _payees) external onlyOwner {
    ...
    */
    ...
    function saturatingAddUint16(uint16 _x, uint16 _y) internal pure returns (uint16) {
...
contract OffchainAggregator is Owned, OffchainAggregatorBilling, AggregatorV2V3Interface, TypeAndVersionInterface {
    ...
    */
     ...
     * @param threshold maximum number of faulty/dishonest oracles the protocol can tolerate while still working
     * correctly
    ...
    modifier checkConfigValid(uint256 _numSigners, uint256 _numTransmitters, uint256 _threshold) {
        ...
        onlyOwner
    {
        ...
        for (uint256 i = 0; i < _signers.length; i++) {
            ...
            require(s_oracles[_signers[i]].role == Role.Unset, "repeated signer address");
            s_oracles[_signers[i]] = Oracle(uint8(i), Role.Signer);
            require(s_payees[_transmitters[i]] != address(0), "payee must be set");
            require(s_oracles[_transmitters[i]].role == Role.Unset, "repeated transmitter address");
        ...
        uint64 configCount = s_configCount;
        {
            s_hotVars.latestConfigDigest = configDigestFromConfigData(
                address(this), configCount, _signers, _transmitters, _threshold, _encodedConfigVersion, _encoded
            );
    ...
    */
     ...
     */
    function setValidatorConfig(AggregatorValidatorInterface _newValidator, uint32 _newGasLimit) public onlyOwner {
        ValidatorConfig memory previous = s_validatorConfig;

        if (previous.validator != _newValidator || previous.gasLimit != _newGasLimit) {
            s_validatorConfig = ValidatorConfig({ validator: _newValidator, gasLimit: _newGasLimit });
    ...
    function validateAnswer(uint32 _aggregatorRoundId, int256 _answer) private {
        ...
        try vc.validator.validate{ gas: vc.gasLimit }(
            prevAggregatorRoundId, prevAggregatorRoundAnswer, _aggregatorRoundId, _answer
        ) { } catch { }
    }
    ...
    */
    ...
    function decodeReport(bytes memory _report)
        internal
        pure
        returns (bytes32 rawReportContext, bytes32 rawObservers, int192[] memory observations)
    {
    ...
    */
        ...
        returns (uint256 length)
    {
        ...
        return uint256(TRANSMIT_MSGDATA_CONSTANT_LENGTH_COMPONENT) + _report.length // one byte pure entry in _report
     ...
     * @param _report serialized report, which the signatures are signing. See parsing code below for format. The ith
     * element of the observers component must be the index in s_signers of the address for the ith signature
     * @param _rs ith element is the R components of the ith signature on report. Must have at most maxNumOracles
     * entries
     * @param _ss ith element is the S components of the ith signature on report. Must have at most maxNumOracles
     * entries
        ...
        external
    {
        ...
        // zero tail.
        require(msg.data.length == expectedMsgDataLength(_report, _rs, _ss), "transmit message too long");
        ReportData memory r; // Relieves stack pressure
        {
            r.hotVars = s_hotVars; // cache read from storage

            bytes32 rawObservers;
            (r.rawReportContext, rawObservers, r.observations) = abi.decode(_report, (bytes32, bytes32, int192[]));
            ...
            require(r.hotVars.latestConfigDigest == configDigest, "configDigest mismatch");
            ...
            require(r.observations.length <= maxNumOracles, "num observations out of bounds");
            require(r.observations.length > 2 * r.hotVars.threshold, "too few values to trust median");
...

        {
            ...
            s_transmissions[r.hotVars.latestAggregatorRoundId] = Transmission(median, uint64(block.timestamp));

            emit NewTransmission(
                r.hotVars.latestAggregatorRoundId, median, msg.sender, r.observations, r.observers, r.rawReportContext
            );
            ...
            emit AnswerUpdated(median, r.hotVars.latestAggregatorRoundId, block.timestamp);
